...
\begin{document}
	...
	Si $\vec{P}$ tiene fuentes vectoriales, tendríamos rotacional de $\vec{D}$. (Lo puedo probar, recordemos que estamos en electroestática). $\nabla \times \vec{D} = \nabla \times( \epsilon_0 \vec{E} + \vec{P})$, por electroestática, el rotacional del campo eléctrico es nulo, $ \nabla \times \vec{D} = \nabla \times \vec{P}$. Entonces puedo calcular $\nabla \times \vec{P}$ para ver si tengo fuentes vectoriales.
	...
	\noindent \textbf{Problema 4.15} Una corteza esférica gruesa (radio interior $a$, radio exterior $b$) está hecha de material dieléctrico con una polarización ''congelad''.
